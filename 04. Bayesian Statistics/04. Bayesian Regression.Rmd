...
m_lwage_expr = lm(lwage ~ iq, data = wage)
model <- augment(m_lwage_expr)
...
outliers_df %>%
  filter(probability < 0.90 & probability > 0.5)
```
...
outliers_df <- data.frame(probability = outliers$prob.outlier,
                          case = 1:length(outliers$prob.outlier))
outliers_df %>% filter(probability > 0.95)
```
...
m_lwage <- lm(lwage ~ . - wage, data = na.omit(wage))
stepAIC(m_lwage, direction="both", scope= ~., k=log(n))
```
...
variable.names(BPM_pred_lwage) %>% intersect(variable.names(HPM_pred_lwage)) %>% intersect(variable.names(MPM_pred_lwage))
```
...
ci_bpm_lwage
opt_bma <- which.max(BPM_pred_lwage$fit)
exp(ci_bpm_lwage[opt_bma, ])
```
