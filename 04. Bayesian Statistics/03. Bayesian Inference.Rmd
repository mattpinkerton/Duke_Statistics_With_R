...
bayes_inference(y = weight, data = nc_fullterm, 
                ...
                show_plot = FALSE, prior_family = "JZS", mu_0 = 7.7, rscale = 1,
                method = "theoretical")
```
...
female <- nc_fullterm %>% filter(gender=="female")
ggplot(female, aes(x = habit, y = weight)) +
  geom_boxplot()
male <- nc_fullterm %>% filter(gender=="male")
ggplot(male, aes(x = habit, y = weight)) +
  geom_boxplot()
```
...
pss = PrisonStress %>%
  mutate(diff = PSSbefore - PSSafter)
bayes_inference(y = diff, x=Group, data = pss, 
                statistic = "mean",
                type = "ht", alternative = "twosided", null = 0, 
                prior = "JZS", rscale = 1, 
                method = "simulation", show_plot = FALSE)
